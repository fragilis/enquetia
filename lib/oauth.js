...
// Configure the twitter strategy for use by Passport.js.
passport.use(new TwitterStrategy({
    consumerKey: config.get('TWITTER_COMSUMER_KEY'),
    consumerSecret: config.get('TWITTER_COMSUMER_SECRET'),
...
function addTemplateVariables(req, res, next) {
  ...
  res.locals.login = `/oauth/login?return=${encodeURIComponent(
    req.originalUrl
  )}`;
  res.locals.logout = `/oauth/logout?return=${encodeURIComponent(
    req.originalUrl
  )}`;
  next();
}
...
router.get(
  ...
  (req, res, next) => {
    if (req.query.return) {
      req.session.oauthreturn = req.query.return;
    }
    next();
  },
...
router.get(
  ...
  (req, res) => {
    const redirect = req.session.oauthreturn || '/';
    delete req.session.oauthreturn;
    req.flash('info', 'ログインしました。画面右上のアイコンまたはユーザー名からマイページに移動できます。');
